...
- [[recorrido_en_grafos_teoria]] ✅ **2C-2026** — Ordenamiento topologico (lema del DAG, teorema aciclico ⟺ orden topologico, algoritmo ingenuo $O(n(n+m))$, ORDEN-TOPOLOGICO/Kahn con cola, deteccion de ciclo por $|L|<n$), esquema general de recorrido (cola⇒BFS / pila⇒DFS), BFS (capas $L_i$, colores, pseudocodigo, lema de la cola, teorema $d[v]=\delta(s,v)$, arbol $G_\pi$, PRINT-PATH), DFS (bosque, tiempos $d/f$, DFS-VISIT, $\Theta(|V|+|E|)$), estructura de parentesis (teorema de los parentesis, corolario de descendiente, teorema del camino blanco), clasificacion de aristas (arbol/retroceso/avance/cruce, regla del color, caso no dirigido), aristas de corte (low, CALCULAR-LOW-OFFLINE, PUENTES + DFS-PUENTES-VISIT, criterio $low[v]>d[u]$, 2 ejemplos trabajados) · 🔄 divergencia de notacion en puentes: $d[]$ (vigente) vs nivel (guia) vs cubren (practica) · `raw/cursada_2C_2026/teo/teo_clase2_algo_en_grafos.pdf`
